...
def _facilitator_verify(payment_header: str, requirements: dict) -> dict:
    ...
    try:
        ...
        print(
            f"[x402] verify status={verify_resp.status_code} body={verify_data}",
            flush=True,
        )
        if not verify_data.get("isValid", False):
            reason = (
                verify_data.get("invalidReason")
                or verify_data.get("error")
        ...
        print(
            f"[x402] settle status={settle_resp.status_code} body={settle_data}",
            flush=True,
        )
        if settle_resp.status_code != 200 or not settle_data.get("success", False):
            reason = (
                settle_data.get("error")
                or f"settle failed (HTTP {settle_resp.status_code})"
            )
