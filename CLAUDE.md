...
2. `joinChallenge()` — 挑战期内，前端签名 SPL Token 转账交易，后端提交上链收取押金
...
Anchor programs: `programs/challenge-escrow/src/lib.rs`, `programs/staking-vault/src/lib.rs` (Solana, Anchor framework)
Devnet program IDs: see `Anchor.toml`
...
Frontend signs Solana SPL Token transfer transaction (`@solana/web3.js` + `@solana/spl-token`) → base64 payload in `X-PAYMENT` header → backend decodes and verifies via `x402.org/facilitator`. Bounty=0 skips payment entirely.

Key details: network is `solana-devnet`, USDC mint is `4zMMC9srt5Ri5X14GAgXhaHii3GnPAEERYPJgZJDncDU` (Circle Devnet USDC), httpx needs `follow_redirects=True` (308 redirect).
...
Blockchain calls (solana-py / solders payout) are always mocked — no real chain interaction in tests.
...
| `USDC_MINT` | `4zMMC9srt5Ri5X14GAgXhaHii3GnPAEERYPJgZJDncDU` | Circle Devnet USDC mint |
| `PLATFORM_WALLET` | (none) | Platform Solana pubkey (Base58) |
| `PLATFORM_PRIVATE_KEY` | (none) | Platform keypair (JSON byte array, 64 bytes) |
| `ESCROW_PROGRAM_ID` | (none) | Deployed ChallengeEscrow Anchor program |
| `STAKING_PROGRAM_ID` | (none) | Deployed StakingVault Anchor program |
| `FACILITATOR_URL` | `https://x402.org/facilitator` | x402 verification endpoint |
| `X402_NETWORK` | `solana-devnet` | Network identifier |
| `NEXT_PUBLIC_DEV_WALLET_KEY` | (in `.env.local`) | DevPanel signing key (base64) |
