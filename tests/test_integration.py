...
def test_fastest_first_full_lifecycle(client):
    ...
    with patch("app.routers.submissions.invoke_oracle"):
        sub_a = client.post(
            ...
            json={"worker_id": "agent-A", "content": "My answer"},
        ).json()
    ...
    with patch("app.routers.submissions.invoke_oracle"):
        sub_b = client.post(
            f"/tasks/{task['id']}/submissions",
            json={"worker_id": "agent-B", "content": "Another answer"},
        ).json()
    ...
    client.post(
        f"/internal/submissions/{sub_b['id']}/score",
        json={"score": 0.95, "feedback": "Perfect"},
    )
...
def test_quality_first_full_lifecycle(client):
    ...
    with PAYMENT_MOCK:
        task = client.post(
            ...
            json={
                "title": "Quality wins",
                "description": "Refine your answer",
                "type": "quality_first",
                "max_revisions": 3,
                "deadline": future(),
                "publisher_id": "test-pub",
    ...
    with patch("app.routers.submissions.invoke_oracle"):
        r1 = client.post(
            f"/tasks/{task['id']}/submissions",
            json={"worker_id": "agent-X", "content": "Draft 1"},
        ).json()
    ...
    with patch("app.routers.submissions.invoke_oracle"):
        r2 = client.post(
            f"/tasks/{task['id']}/submissions",
            json={"worker_id": "agent-X", "content": "Better draft"},
        ).json()
...
def test_filter_tasks_by_status(client):
    with PAYMENT_MOCK:
        with patch("app.routers.submissions.invoke_oracle"):
            ...
            sub = client.post(
                f"/tasks/{t1['id']}/submissions",
                json={"worker_id": "w", "content": "x"},
            ).json()
...
def test_bounty_lifecycle_fastest_first(client):
    ...
    pub = client.post(
        ...
        json={
            "nickname": "pub-int",
            "wallet": "PUBintSoL111111111111111111111111111111112",
            "role": "publisher",
        },
    ).json()
    worker = client.post(
        ...
        json={
            "nickname": "worker-int",
            "wallet": "WORKERintSoL1111111111111111111111111111112",
            "role": "worker",
        },
    ).json()
    ...
    with patch("app.routers.submissions.invoke_oracle"):
        sub = client.post(
            f"/tasks/{task['id']}/submissions",
            json={"worker_id": worker["id"], "content": "my answer"},
        ).json()

    # 4. Score above threshold -> triggers payout
    with patch(
        "app.services.payout._send_usdc_transfer", return_value="0xPAYOUT"
    ) as mock_tx:
        client.post(f"/internal/submissions/{sub['id']}/score", json={"score": 0.9})
        mock_tx.assert_called_once_with(
            "WORKERintSoL1111111111111111111111111111112", 8.0
        )  # 10.0 * 0.80
...
def test_bounty_lifecycle_quality_first(client):
    ...
    pub = client.post(
        ...
        json={
            "nickname": "pub-q",
            "wallet": "PUBqSoL1111111111111111111111111111111112",
            "role": "publisher",
        },
    ).json()
    worker = client.post(
        ...
        json={
            "nickname": "worker-q",
            "wallet": "WORKERqSoL111111111111111111111111111111112",
            "role": "worker",
        },
    ).json()
    ...
    with patch("app.routers.submissions.invoke_oracle"):
        r1 = client.post(
            f"/tasks/{task['id']}/submissions",
            json={"worker_id": worker["id"], "content": "draft 1"},
        ).json()
        r2 = client.post(
            f"/tasks/{task['id']}/submissions",
            json={"worker_id": worker["id"], "content": "draft 2"},
        ).json()
