...
def test_full_trust_settlement_upheld(client):
    ...
    w_sub = Submission(
        task_id=task.id, worker_id=winner.id, content="w", score=0.9, status="scored"
    )
    c_sub = Submission(
        task_id=task.id,
        worker_id=challenger.id,
        content="c",
        score=0.7,
        status="scored",
    )
    ...
    ch = Challenge(
        task_id=task.id,
        challenger_submission_id=c_sub.id,
        target_submission_id=w_sub.id,
        reason="Better",
        status=ChallengeStatus.judged,
        challenger_wallet="0xIC",
...
def test_full_trust_settlement_malicious(client):
    ...
    with (
        patch("app.scheduler.resolve_challenge_onchain", return_value="0x"),
...
def test_settlement_no_challenges(client):
    ...
    o_sub = Submission(
        task_id=task.id, worker_id=other.id, content="o", score=0.6, status="scored"
    )
