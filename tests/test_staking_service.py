...
def test_stake_for_arbiter_success(client):
    ...
    with patch("app.services.staking.stake_onchain", return_value="5VERvTx"):
        record = stake_for_arbiter(db, user.id, signed_transaction="dGVzdA==")
...
def test_stake_for_arbiter_not_s_tier(client):
    ...
    with pytest.raises(ValueError, match="S-tier"):
        stake_for_arbiter(db, user.id, signed_transaction="dGVzdA==")
...
def test_stake_for_credit(client):
    ...
    with patch("app.services.staking.stake_onchain", return_value="5VERvTx2"):
        record = stake_for_credit(
            db, user.id, amount=50.0, signed_transaction="dGVzdA=="
        )
...
def test_stake_for_credit_cap(client):
    ...
    with patch("app.services.staking.stake_onchain", return_value="5VERvTx3"):
        record = stake_for_credit(
            db, user.id, amount=50.0, signed_transaction="dGVzdA=="
        )
...
def test_check_and_slash(client):
    ...
    with patch("app.services.staking.slash_onchain", return_value="5VERvSlash"):
