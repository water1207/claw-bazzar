...
def client():
    ...
    with (
        patch("app.main.create_scheduler", return_value=MagicMock()),
...
MOCK_DIM_SCORES = {
    "substantiveness": _make_dim_score_response(
        "substantiveness",
        "实质性",
        [
            {
    ...
    "credibility": _make_dim_score_response(
        "credibility",
        "可信度",
        [
            {
    ...
    "completeness": _make_dim_score_response(
        "completeness",
        "完整性",
        [
            {
    ...
    "tech_depth": _make_dim_score_response(
        "tech_depth",
        "技术深度",
        [
            {
...
class TestFastestFirstE2E:
    ...
    def test_gate_pass_high_score_closes_task(self, db):
        ...
        task = Task(
            ...
            bounty=0.1,
            acceptance_criteria=json.dumps(["覆盖率达到80%"]),
        )
        db.add(task)
        db.commit()
        ...
        with (
            patch("app.services.oracle.subprocess.run", side_effect=mock),
    ...
    def test_gate_pass_low_score_stays_open(self, db):
        ...
        task = Task(
            ...
            deadline=datetime(2026, 12, 31, tzinfo=timezone.utc),
            bounty=0.1,
            acceptance_criteria=json.dumps(["覆盖所有接口"]),
        )
        db.add(task)
        db.commit()
        _create_test_dims(db, task.id)
    ...
    def test_gate_fail_scores_zero(self, db):
        ...
        task = Task(
            ...
            deadline=datetime(2026, 12, 31, tzinfo=timezone.utc),
            bounty=0.1,
            acceptance_criteria=json.dumps(["至少5个竞品"]),
        )
        db.add(task)
        db.commit()
...
class TestQualityFirstE2E:
    ...
    def test_gate_check_and_individual_scoring(self, db):
        ...
        task = self._setup_quality_task(
            db, deadline=datetime(2026, 12, 31, tzinfo=timezone.utc)
        )
    ...
    def test_batch_score_threshold_filter_and_horizontal(self, db):
        ...
        sub_low = self._add_gate_passed_sub(
            db, task.id, "w3", MOCK_INDIVIDUAL_BELOW_THRESHOLD
        )
        ...
        for dim_id, dim_name in [
            ...
            dim_scores_2subs[dim_id] = _make_dim_score_response(
                dim_id,
                dim_name,
                [
                    {
    ...
    def test_full_lifecycle_no_challenge(self, db):
        ...
        for dim_id, dim_name in [
            ...
            dim_score_mock[dim_id] = _make_dim_score_response(
                dim_id,
                dim_name,
                [
                    {
        ...
        with (
            patch("app.services.oracle.subprocess.run", side_effect=mock_run),
    ...
    def test_score_hiding_during_open_and_scoring(self, client):
        ...
        task = Task(
            ...
            deadline=datetime(2026, 12, 31, tzinfo=timezone.utc),
            bounty=0.1,
            acceptance_criteria=json.dumps(["10个产品"]),
            publisher_id=pub.id,
        )
        s.add(task)
        s.commit()
        ...
        sub = Submission(
            ...
            oracle_feedback=json.dumps(
                {"type": "individual_scoring", "dimension_scores": {}}
            ),
        )
        s.add(sub)
        s.commit()
...
class TestQualityFirstChallengeE2E:
    ...
    def test_challenge_upheld_changes_winner(self, db):
        ...
        challenger_user = User(
            nickname="challenger", wallet="0xchallenger", role=UserRole.worker
        )
        ...
        with (
            patch("app.scheduler.run_arbitration", side_effect=mock_arbiter),
...
class TestHTTPE2E:
    ...
    def test_fastest_first_full_http_flow(self, client):
        ...
        resp = client.post(
            ...
            json={
                "nickname": "pub_http",
                "wallet": "PUBhttpSoL11111111111111111111111111111112",
                "role": "publisher",
            },
        )
        ...
        with (
            patch("app.services.oracle.subprocess.run", side_effect=dim_mock),
            PAYMENT_MOCK,
        ):
            resp = client.post(
                "/tasks",
        ...
        resp = client.post(
            "/users",
            json={
                "nickname": "worker_http",
                "wallet": "WORKERhttpSoL111111111111111111111111111112",
                "role": "worker",
            },
        )
        assert resp.status_code == 201
    ...
    def test_quality_first_http_flow_with_score_hiding(self, client):
        ...
        resp = client.post(
            ...
            json={
                "nickname": "pub_qf",
                "wallet": "PUBqfSoL11111111111111111111111111111111112",
                "role": "publisher",
            },
        )
        pub_id = resp.json()["id"]
        ...
        resp = client.post(
            ...
            json={
                "nickname": "worker_qf1",
                "wallet": "WORKERqf1SoL1111111111111111111111111111112",
                "role": "worker",
            },
        )
        w1_id = resp.json()["id"]
